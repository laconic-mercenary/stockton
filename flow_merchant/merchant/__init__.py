...
class Merchant:
    def __init__(self, table_service: TableServiceClient, broker: broker.Broker) -> None:
    ...
    def _handle_signal_when_buying(self, signal: MerchantSignal) -> None:
        ...
        if signal.interval() == self.low_interval():
            if signal.action() == S_ACTION_BUY():
                ## TODO
                take_profit = signal.close() + (signal.close() * 0.05)
    ...
    def last_action_time(self):
        ## this should come from storage
        return self.state.get('last_action_time')
    
    def rest_interval_ms(self):
        ## this should come from merchant config
        return self.state.get('rest_interval')